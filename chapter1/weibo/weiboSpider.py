...
class Weibo:
    cookie = {"Cookie": "SINAGLOBAL=3363210346547.312.1534302424499; UOR=oa.ruijie.com.cn,widget.weibo.com,caibaojian.com; ALF=1541512686; SUB=_2A252vmC9DeRhGedG61cQ8ynFzjyIHXVSQQD1rDV8PUJbkNBeLVPMkW1NUU_MxhgQk3xUV6U_b6aFbG-qZAKuwsLk; SUBP=0033WrSXqPxfM725Ws9jqgMF55529P9D9WFvgdXYBwA14VjJQlY.y2FB5JpX5oz75NHD95Qp1h5feKeN1K-7Ws4DqcjBdgvVIcH4PcSQ9g4AIsLydntt; wvr=6; _s_tentry=-; Apache=2094108038496.4827.1538920715295; ULV=1538920715305:6:2:2:2094108038496.4827.1538920715295:1538913837382"}  # 将your cookie替换成自己的cookie
            ...
            for page in range(1, page_num + 1):
                ...
                if is_empty:
                    for i in range(0, len(info) - 2):
                        ...
                        if a_link:
                            if a_link[-1] == "/comment/" + weibo_id or "/comment/" + weibo_id + "?" in a_link[-1]:
...
def main():
    try:
        ...
        # Dinol: 1257855292 福州日报：1895697240
        user_id = 1895697240  # 可以改成任意合法的用户id（爬虫的微博id除外）
        filter_type = 1  # 值为0表示爬取全部微博（原创微博+转发微博），值为1表示只爬取原创微博
        wb = Weibo(user_id, filter_type)  # 调用Weibo类，创建微博实例wb
        wb.start()  # 爬取微博信息
